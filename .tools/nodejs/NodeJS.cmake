...
function(nodejs_init)
  ...
  set(NODEJS_NAN_DIR ${CMAKE_BINARY_DIR}/node_modules/nan)
  if(NOT EXISTS ${NODEJS_NAN_DIR}/package.json)
    message(STATUS "Install 'nan' for node.js addon building")
    execute_process(
      COMMAND cmd /c npm install nan@latest --prefix .
...
function(add_nodejs_library target NodeVersion)
  cmake_parse_arguments(ARG "" "" "LIBRARIES;SOURCES" ${ARGN})
  set(NODEJS_VERSION ${NodeVersion})
  nodejs_init()

  # Validate name parameter (must be a valid C identifier)
  string(MAKE_C_IDENTIFIER ${target} ${target}_SYMBOL_CHECK)
  if(NOT "${target}" STREQUAL "${${target}_SYMBOL_CHECK}")
      message(FATAL_ERROR
          "Module name must be a valid C identifier. "
          "Suggested alternative: '${${target}_SYMBOL_CHECK}'"
      )
  endif()
  ...
  add_library(${target} SHARED ${NODEJS_SOURCES} ${ARG_SOURCES})
  # Add compiler defines for the module
  # Two helpful ones:
  # MODULE_NAME must match the name of the build library, define that here
  # ${target}_BUILD is for symbol visibility under windows
  string(TOUPPER "${target}_BUILD" ${target}_BUILD_DEF)
  target_compile_definitions(${target}
      PRIVATE MODULE_NAME=${target}
      PRIVATE ${${target}_BUILD_DEF}
      PUBLIC ${NODEJS_DEFINITIONS}
  )
  # This properly defines includes for the module
  target_include_directories(${target} PUBLIC ${NODEJS_INCLUDE_DIRS})

  # Add link flags to the module
  target_link_libraries(${target} ${NODEJS_LIBRARIES} ${ARG_LIBRARIES})
  ...
  foreach(NODEJS_LINK_FLAG IN LISTS NODEJS_LINK_FLAGS)
      set_property(TARGET ${target} APPEND_STRING PROPERTY LINK_FLAGS " ${NODEJS_LINK_FLAG}")
  endforeach()
