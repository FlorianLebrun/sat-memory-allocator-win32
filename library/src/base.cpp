...
void sat_begin_stack_beacon(SAT::StackBeacon* beacon) {
   SAT::Thread* thread = SAT::current_thread;
   if (!thread) thread = (SAT::Thread*)g_SAT.getCurrentThread();
   if (thread->stackBeacons >= thread->stackBeaconsBase) {
      beacon->beaconId = ++thread->stackLastId;
      (--thread->stackBeacons)[0] = beacon;
   }
   else {
...
void sat_end_stack_beacon(SAT::StackBeacon* beacon) {
   SAT::Thread* thread = SAT::current_thread;
   if (!thread) thread = (SAT::Thread*)g_SAT.getCurrentThread();
   while (thread->stackBeacons[0] && uintptr_t(thread->stackBeacons[0]) < uintptr_t(beacon)) {
      thread->stackBeacons++;
      printf("Error in SAT stack beacon: a previous beacon has been not closed.\n");
   }
   if (thread->stackBeacons[0] && thread->stackBeacons[0] == beacon) {
      thread->stackBeacons++;
   }
   else if(thread->stackBeacons != thread->stackBeaconsBase) {
