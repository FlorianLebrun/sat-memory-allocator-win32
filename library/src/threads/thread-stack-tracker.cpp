...
namespace SAT {
   ...
   bool ThreadStackTracker::captureFrames(std::function<void(StackFrame*)> processing) {
      ...
      if (context.capture(this->thread)) {
         bool success = this->updateFrames(context, this->thread->stackBeacons);
   ...
   uint64_t Thread::getStackStamp() {
      ...
      if (g_SAT.stackStampDatabase && isCurrentThread) {
         ...
         if (!this->stackTracker.updateFrames(context, this->stackBeacons)) {
            return 0;
         }
         ...
         int stackSize = 0;
         for (StackFrame* frame = this->stackTracker.lastStack; frame; frame = frame->next) {
            if (frame->node) {
