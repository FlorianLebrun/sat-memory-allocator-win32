...
#include <windows.h>
extern"C" void __nop();
void sat::SpinLock::SpinlockWait(volatile tSpinWord *w, int32_t value, int loop) {
   if (loop == 0) {
   } else if (loop == 1) {
      Sleep(0);
   } else {
...
void sat::SpinLock::SpinlockPause() {
  __nop();
}
...
#else
  #error "'nop' instruction not managed."
#endif
}
