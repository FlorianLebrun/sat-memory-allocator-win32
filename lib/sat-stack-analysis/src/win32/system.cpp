...
void SystemThreading::SleepThread(int time_us) {
   ...
   uint64_t ncycle = uint64_t(double(time_us) * 1000000.0 / double(freq));
   uint64_t endTime = SystemThreading::GetTimeStamp() + ncycle;
   if (time_us > 2000) {
      ::Sleep(time_us / 1000);
   }
...
bool SystemThreading::tSymbol::resolve(uintptr_t address)
{
   ...
   IMAGEHLP_SYMBOL64* Sym = (IMAGEHLP_SYMBOL64*)_malloca(sizeof(IMAGEHLP_SYMBOL64) + this->nameLen);
   if (!Sym) throw;
   uint64_t offset;
   ...
   if (SymGetSymFromAddr64(GetCurrentProcess(), address, &offset, Sym)) {
      ...
      sprintf_s(this->name, this->nameLen, "(unresolved 0x%.16llX : code %d)", int64_t(address), GetLastError());
      return false;
   }
   else {
      this->name[0] = 0;
